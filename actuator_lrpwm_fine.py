...
time_array = np.linspace(0, CONTROL_STEP, CONTROL_STEP*PWM_FREQUENCY*n + 1, endpoint=True)
...
for i in range(len(stateArray)-1):
    ...
    for j in range(n_factor):
        ...
        state_i[0:4] += (k1_q + 2.0 * k2_q + 2.0 * k3_q + k4_q)/6.0  # update step for q
        state_i[0:4] /= np.linalg.norm(stateArray[i+1, 1:4])    # normalising q
        state_i[4:7] += 1/6.0 * (k1_w + 2.0 * k2_w + 2.0 * k3_w + k4_w)     # update step for w
    stateArray[i+1, :] = state_i
np.savetxt("actuator_pwm_state_nyquist.csv", stateArray[:, :], delimiter=",")
