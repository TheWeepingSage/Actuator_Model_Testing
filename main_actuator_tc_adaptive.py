...
time_temp = np.zeros(2*len(time)-1)
for i in range(len(time)-1):
    time_temp[2*i] = time[i]
    time_temp[2*i + 1] = (time[i] + time[i+1])/2
time_temp[2*len(time) - 2] = time[len(time)-1]
time = time_temp
num_instants = len(time_temp + 1)
...
for i in range(0, num_instants - 1):
    ...
    if i % (num_instants / 100) == 0:
        ...
        end_1 = timer.time()
        print(end_1 - start_1)
np.savetxt("aac_test_tc_adaptive.csv", state_array[:, :], delimiter=",")
end = timer.time()
print(-start + end)
